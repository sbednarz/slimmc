...
def test_composition_filters_work_in_composition_and_full_modes():
    for full in (False, True):
        ...
        try:
            ...
            assert c.where_fraction('A', min=0.6).mass_distribution(weighting='number').mn > 0
        finally:
            td.cleanup()
...
def test_run_plot_general_namespace_shortcuts():
    ...
    try:
        ...
        assert run.plot.mass_distribution(weighting='number').get_xlabel()
        assert run.plot.cld(weighting='number').get_xlabel()
