...
suite "homo scheduling/actions/conditions phase C":
  test "H11 parses at, every-from-zero, and shifted from/step schedules":
    let path = writeTempModel("slimmc_phase_c_schedule.model", base & """
at 0.0 save
at 0.25 set_k k 2.0
...
""")
  ...
  test "H11 rejects non-positive every period and scheduled stop":
    for line in ["every 0 save", "every -1 save", "from -1 step 1 save", "from 0 step 0 save", "from 0 save", "at 0.2 stop"]:
      let path = writeTempModel("slimmc_phase_c_bad_schedule.model", base & line & "\n")
      expect(ValueError):
        discard parseModel(path)
      removeFile(path)

  test "H12 parses set/add rate and temperature actions":
    let path = writeTempModel("slimmc_phase_c_rates.model", base & """
at 0.1 set_k k 2.5
at 0.2 add_k k -0.5
at 0.3 set_temp 320
at 0.4 add_temp 5
""")
  ...
  test "H13 parses set_c and signed add_c":
    let path = writeTempModel("slimmc_phase_c_conc.model", base & """
at 0.1 set_c Q 0.3
at 0.2 add_c Q 0.1
at 0.3 add_c Q -0.05
""")
  ...
  test "H14 parses AND conditions and keeps separate when lines as independent actions":
    let path = writeTempModel("slimmc_phase_c_when.model", base & """
when X M > 0.1 and c Q < 0.15 set_k k 0
...
""")
  ...
  test "H14 rejects unsupported comparison and incomplete condition":
    for line in [
      ...
      let path = writeTempModel("slimmc_phase_c_bad_when.model", base & line & "\n")
      expect(ValueError):
        discard parseModel(path)
      removeFile(path)

  test "mass_model accepts only the two canonical spellings":
    for value in ["repeat_units", "with_end_groups"]:
      let path = writeTempModel("slimmc_mass_model_good.model", base &
        "param mass_model " & value & "\n")
      discard parseModel(path)
      removeFile(path)

    for value in ["with_endgroups", "end_groups", "WITH_END_GROUPS"]:
      let path = writeTempModel("slimmc_mass_model_bad.model", base &
        "param mass_model " & value & "\n")
      expect(ValueError):
        discard parseModel(path)
      removeFile(path)

  test "Storage schema publishes monomer repeat-unit mass":
    let path = writeTempModel("slimmc_homo_monomer_dictionary.model", base)
    let txt = $schemaRecords(parseModel(path))
    check txt.contains("\"dictionary\":\"monomers\"")
    check txt.contains("\"molar_mass_increment\":100.0")
    removeFile(path)


suite "homo simulation-time tolerance":
